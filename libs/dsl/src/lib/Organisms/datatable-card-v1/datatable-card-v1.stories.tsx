/* eslint-disable @nrwl/nx/enforce-module-boundaries */
import {Meta, Story} from '@storybook/react';
import themes from 'libs/dsl/src/theme';
import TableCellComponentV2 from '../../Molecules/table-cell-component-v2/table-cell-component-v2';
import DatatableCardV1, {IDatatableCardV1Props} from './datatable-card-v1';

export default {
  component: DatatableCardV1,
  title: 'Organisms/Table',
  argTypes: {
    height: {
      control: {type: 'range', min: 4, max: 1000, step: 4},
    },
    width: {
      control: {type: 'range', min: 4, max: 1000, step: 4},
    },
    rowClicked: {},
  },
} as Meta;

const Template: Story<IDatatableCardV1Props> = (args) => (
  <DatatableCardV1 {...args} />
);
export const CustomerTable = Template.bind({});
CustomerTable.args = {
  headerData: {
    title: 'card header',
    searchData: {
      label: 'Search',
      placeholder: 'Search by Customer Name, SSE ID, Phone Numbe',
      value: '',
    },
    cardExtraProps: {
      title_color: themes.default.palette.text.tex600,
      bottom_border_color: themes.default.palette.neutral.neu100,
      background_color: themes.default.palette.background.bacopWhite,
    },

    actions: [
      {
        menu: 'Filter',
        icon: {
          name: 'filter_alt_black_24dp',
          size: 25,
          color: themes.default.palette.neutral.neu400,
          label: 'Filter',
        },
        submenu: [],
      },
      {
        menu: 'Download',
        icon: {
          name: 'file_upload_black_24dp-1',
          size: 25,
          color: themes.default.palette.neutral.neu400,
          label: 'Download',
        },
        submenu: [],
      },
    ],
  },
  height: 300,
  width: 400,
  chartData: {
    columnResizable: true,
    pagination: true,
    showBorder: true,
    rowData: [
      {
        customer: {
          userInfo: {
            userAvatar: {
              value: 'Laxman Agarwal',
              path: 'data:image/png;base64,',
            },
            subTitle: 'SSEFL12345',
            chipData: [
              {
                chipColor: themes.default.palette?.secondary?.sec300Main,
                textData: {
                  text: 'New',
                  color: themes.default.palette?.text?.tex300Main,
                  variant: 'overline',
                },
              },
            ],
          },
        },

        projectStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Pending verifcation',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        permitStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.secondary?.sec100,
              textData: {
                text: 'LC Permit',
                color: themes.default.palette?.secondary?.sec300Main,
              },
            },
          ],
        },
        nameChange: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Yes',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        projectType: {
          text: {value: 'Residential(Subsidy)', variant: 'body2'},
        },

        phoneNumber: {text: {value: '(480)555-0103', variant: 'body2'}},
        assignTo: {
          leftAvatar: {
            value: 'Laxman Agarwal',
            path: 'data:image/png;base64,',
          },
        },
        city: {
          text: {
            value: 'East DElhi',
            variant: 'body2',
          },
        },
      },
      {
        customer: {
          userInfo: {
            userAvatar: {
              value: 'Laxman Agarwal',
              path: 'data:image/png;base64,',
            },
            subTitle: 'SSEFL12345',
            chipData: [
              {
                chipColor: themes.default.palette?.secondary?.sec300Main,
                textData: {
                  text: 'New',
                  color: themes.default.palette?.text?.tex300Main,
                  variant: 'overline',
                },
              },
            ],
          },
        },

        projectStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor4?.sys100,
              textData: {
                text: 'Pending verifcation',
                color: themes.default.palette?.systemColor4?.sys300Main,
              },
            },
          ],
        },
        permitStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'PC Permit',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        nameChange: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Yes',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        projectType: {
          text: {value: 'Residential(Subsidy)', variant: 'body2'},
        },

        phoneNumber: {text: {value: '(480)555-0103', variant: 'body2'}},
        assignTo: {
          leftAvatar: {
            value: 'Laxman Agarwal',
            path: 'data:image/png;base64,',
          },
        },
        city: {
          text: {
            value: 'East DElhi',
            variant: 'body2',
          },
        },
      },
      {
        customer: {
          userInfo: {
            userAvatar: {
              value: 'Laxman Agarwal',
              path: 'data:image/png;base64,',
            },
            subTitle: 'SSEFL12345',
            chipData: [
              {
                chipColor: themes.default.palette?.secondary?.sec300Main,
                textData: {
                  text: 'New',
                  color: themes.default.palette?.text?.tex300Main,
                  variant: 'overline',
                },
              },
            ],
          },
        },

        projectStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Pending verifcation',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        permitStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.secondary?.sec100,
              textData: {
                text: 'LC Permit',
                color: themes.default.palette?.secondary?.sec300Main,
              },
            },
          ],
        },
        nameChange: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Yes',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        projectType: {
          text: {value: 'Residential(Subsidy)', variant: 'body2'},
        },

        phoneNumber: {text: {value: '(480)555-0103', variant: 'body2'}},
        assignTo: {
          leftAvatar: {
            value: 'Laxman Agarwal',
            path: 'data:image/png;base64,',
          },
        },
        city: {
          text: {
            value: 'East DElhi',
            variant: 'body2',
          },
        },
      },
      {
        customer: {
          userInfo: {
            userAvatar: {
              value: 'Laxman Agarwal',
              path: 'data:image/png;base64,',
            },
            subTitle: 'SSEFL12345',
            chipData: [
              {
                chipColor: themes.default.palette?.secondary?.sec300Main,
                textData: {
                  text: 'New',
                  color: themes.default.palette?.text?.tex300Main,
                  variant: 'overline',
                },
              },
            ],
          },
        },

        projectStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor4?.sys100,
              textData: {
                text: 'Pending verifcation',
                color: themes.default.palette?.systemColor4?.sys300Main,
              },
            },
          ],
        },
        permitStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'PC Permit',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        nameChange: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Yes',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        projectType: {
          text: {value: 'Residential(Subsidy)', variant: 'body2'},
        },

        phoneNumber: {text: {value: '(480)555-0103', variant: 'body2'}},
        assignTo: {
          leftAvatar: {
            value: 'Laxman Agarwal',
            path: 'data:image/png;base64,',
          },
        },
        city: {
          text: {
            value: 'East DElhi',
            variant: 'body2',
          },
        },
      },
      {
        customer: {
          userInfo: {
            userAvatar: {
              value: 'Laxman Agarwal',
              path: 'data:image/png;base64,',
            },
            subTitle: 'SSEFL12345',
            chipData: [
              {
                chipColor: themes.default.palette?.secondary?.sec300Main,
                textData: {
                  text: 'New',
                  color: themes.default.palette?.text?.tex300Main,
                  variant: 'overline',
                },
              },
            ],
          },
        },

        projectStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Pending verifcation',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        permitStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.secondary?.sec100,
              textData: {
                text: 'LC Permit',
                color: themes.default.palette?.secondary?.sec300Main,
              },
            },
          ],
        },
        nameChange: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Yes',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        projectType: {
          text: {value: 'Residential(Subsidy)', variant: 'body2'},
        },

        phoneNumber: {text: {value: '(480)555-0103', variant: 'body2'}},
        assignTo: {
          leftAvatar: {
            value: 'Laxman Agarwal',
            path: 'data:image/png;base64,',
          },
        },
        city: {
          text: {
            value: 'East DElhi',
            variant: 'body2',
          },
        },
      },
      {
        customer: {
          userInfo: {
            userAvatar: {
              value: 'Laxman Agarwal',
              path: 'data:image/png;base64,',
            },
            subTitle: 'SSEFL12345',
            chipData: [
              {
                chipColor: themes.default.palette?.secondary?.sec300Main,
                textData: {
                  text: 'New',
                  color: themes.default.palette?.text?.tex300Main,
                  variant: 'overline',
                },
              },
            ],
          },
        },

        projectStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor4?.sys100,
              textData: {
                text: 'Pending verifcation',
                color: themes.default.palette?.systemColor4?.sys300Main,
              },
            },
          ],
        },
        permitStatus: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'PC Permit',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        nameChange: {
          sqchip: [
            {
              chipColor: themes.default.palette?.systemColor2?.sys100,
              textData: {
                text: 'Yes',
                color: themes.default.palette?.systemColor2?.sys300Main,
              },
            },
          ],
        },
        projectType: {
          text: {value: 'Residential(Subsidy)', variant: 'body2'},
        },

        phoneNumber: {text: {value: '(480)555-0103', variant: 'body2'}},
        assignTo: {
          leftAvatar: {
            value: 'Laxman Agarwal',
            path: 'data:image/png;base64,',
          },
        },
        city: {
          text: {
            value: 'East DElhi',
            variant: 'body2',
          },
        },
      },
    ],
    columnData: [
      {
        field: 'customer',
        headerName: 'Customer',
        flex: 1,
        sortable: false,
        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              userInfo={values.value.userInfo}
            ></TableCellComponentV2>
          );
        },
      },
      {
        headerName: 'Project Status',
        field: 'projectStatus',
        flex: 1,
        editable: false,
        sortable: false,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              sqchip={values.value.sqchip}
            ></TableCellComponentV2>
          );
        },
      },

      {
        headerName: 'Permit Status',
        field: 'permitStatus',
        flex: 1,
        editable: false,
        sortable: false,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              sqchip={values.value.sqchip}
            ></TableCellComponentV2>
          );
        },
      },
      {
        headerName: 'Phone Number',
        field: 'phoneNumber',
        flex: 1,
        editable: false,
        sortable: false,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              text={values.value.text}
            ></TableCellComponentV2>
          );
        },
      },
      {
        headerName: 'Assign to',
        field: 'assignTo',
        flex: 1,
        editable: false,
        sortable: false,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              primaryAvatar={values.value.leftAvatar}
            ></TableCellComponentV2>
          );
        },
      },
      {
        headerName: 'City',
        field: 'city',
        editable: false,
        sortable: false,
        flex: 1,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              text={values.value.text}
            ></TableCellComponentV2>
          );
        },
      },
      {
        headerName: 'Project Type',
        field: 'projectType',
        editable: false,
        sortable: false,
        flex: 1,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              text={values.value.text}
            ></TableCellComponentV2>
          );
        },
      },

      {
        headerName: 'Name Change',
        field: 'nameChange',
        editable: false,
        sortable: false,
        flex: 1,

        renderCell: (values: any) => {
          return (
            <TableCellComponentV2
              sqchip={values.value.sqchip}
            ></TableCellComponentV2>
          );
        },
      },
    ],
  },
  chartProps: {
    background_color: themes.default?.palette?.background?.bacopWhite,
  },
};
